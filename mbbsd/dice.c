/* $Id: dice.c 2796 2005-06-08 02:46:24Z piaip $ */
#include "bbs.h"
...
del(int value[100],int total, dicedata_t * table)
{
	...
	for (i = 0; i < total; i++) {
		...
		move(19, 0);
		clrtoeol();
		prints("你現在有 %u NH$歐", cuser.money);
...
dice_main(void)
{
	...
	while (1) {
	    move(19, 0);
	    prints(ANSI_COLOR(1;32) "你現在有" ANSI_COLOR(1;31) " %u " ANSI_COLOR(1;32) "NH$歐" ANSI_RESET,
		   cuser.money);
	    ...
	    reload_money();
	    move(19, 0);
	    prints(ANSI_COLOR(1;32) "你現在有 " ANSI_COLOR(1;31) "%u" ANSI_COLOR(1;32) " NH$歐",
		   cuser.money);
	    if (sig != 2)
		show_count(value,index, money);
	}
	...
	if (total > 0) {
	    move(21, 0);
	    prints(ANSI_COLOR(1;32) "你贏了 " ANSI_COLOR(1;31) "%d" ANSI_COLOR(1;32) " NH$ 唷~~"
		   "                    " ANSI_RESET, total);
	} else {
	    move(21, 0);
	    clrtoeol();
