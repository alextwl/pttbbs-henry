/* $Id: guess.c 2796 2005-06-08 02:46:24Z piaip $ */
#include "bbs.h"
...
get_money(void)
{
    ...
    clrtoeol();
    reload_money();
    prints("您目前有:%d NH$", cuser.money - money);
    return money;
}
...
guess_main(void)
{
	...
	if (count < 5) {
	    outs("真厲害, 錢被你賺走了");
	    if ((file = fopen(LOGPASS, "a"))) {
		fprintf(file, "id: %s, 第%d次猜中, 贏了 %d NH$\n",
	    ...
	    outs("唉, 太多次才猜出來了");
	    if ((file = fopen(LOGPASS, "a"))) {
		fprintf(file, "id: %s, 第%d次才猜中, 賠了 %d NH$\n",
	    ...
	    prints("你拿回了%d NH$\n", money);
	    if ((file = fopen(LOGPASS, "a"))) {
		fprintf(file, "id: %s, 第%d次猜中, 拿回了本錢 %d NH$\n",
			cuser.userid, count, money);
		fclose(file);
	    }
	}
